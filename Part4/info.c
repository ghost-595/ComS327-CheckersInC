...
void textToBoard(char text[], char **board, int *boardKeywordPosition, int *currentLine, int *error);

FILE* infile;

int validateText(char text[], char **board, char **moves, char *turn, int *noCapture, int *multipuleJumps)
{
  ...
  for(wordCount = 1;text[position] != '\0';)
  {
    ...
    if(strlen(word) > 0)
    {
      ...
      else if(strcmp(word, "BOARD:") == 0 && wordCount == 6)
      {
        ...
        textToBoard(text, board, &position, &currentLine, &error);
        if(error == 1)
        {
          return 1;
        }
      ...
      if(movesFound == 1)
      {
        ...
        if(strlen(word) != 6)
        {
          fprintf(stderr,"incorrect amount of charcters for move on line %d\n", currentLine);
          return 1;
        }
...
void textToBoard(char text[], char **board, int *position, int *currentLine, int *error)
{
    ...
    for(int r = 0; r < 8;)
    {
      ...
      if(isspace(tmpChar) == 0 && tmpChar != '|' && tmpChar != '-' && tmpChar != '+')
      {
        ...
        if(tmpChar == 'b' || tmpChar == 'B' || tmpChar == '.' || tmpChar == 'r' || tmpChar == 'R' )
        {
          ...
          {
            *error = 1;
            fprintf(stderr,"two black squares are in a row on line %d\n", *currentLine);
            return;
          }
          else
          {
        ...
        else if(tmpChar == '\"')
        {
          ...
          {
            *error = 1;
            fprintf(stderr,"two red squares are in a row on line %d\n", *currentLine);
            return;
          }
          else
          {
